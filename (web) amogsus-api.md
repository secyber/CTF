...
Another way, wrapping everything in just double-quotes (which in some cases would be best), would be to hijack a user (in this case of `id=1`) and change it so that we are able to login with new credentials and `sus=1`. We know this from the `.db` file.  In this case, the query is:
```
